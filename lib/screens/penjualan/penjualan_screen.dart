...
import 'penjualan_form_sheet.dart';
import 'penjualan_detail_sheet.dart';

class PenjualanScreen extends StatefulWidget {
  final bool embedded;
  const PenjualanScreen({super.key, this.embedded = false});
...
class _PenjualanScreenState extends State<PenjualanScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      setState(() {
        _error = e is ApiException ? e.message : 'Gagal memuat data penjualan';
        _loading = false;
      });
    }
  }

  Future<void> _openForm({Penjualan? existing}) async {
    final saved = await showModalBottomSheet<bool>(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (_) => PenjualanFormSheet(existing: existing),
    );
    if (saved == true) _load();
  }

  Future<void> _openDetail(Penjualan p) async {
    await showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (_) => PenjualanDetailSheet(id: p.id),
    );
  }

  Future<void> _cancel(Penjualan p) async {
    final confirmed = await AppFeedback.confirm(
      context,
      title: 'Batalkan Transaksi',
      message: 'Batalkan transaksi ${p.nomor}? Stok barang akan dikembalikan.',
      danger: true,
    );
    if (!confirmed) return;
    try {
  ...
  Widget build(BuildContext context) {
    ...
    final canManage = can(role, 'penjualan.manage');
    ...
    final body = RefreshIndicator(
      ...
      child: _loading
          ...
          : _error != null
              ...
              : _items.isEmpty
                  ? const EmptyView(message: 'Belum ada transaksi penjualan.', icon: Icons.point_of_sale_outlined)
                  : ListView.separated(
                      ...
                      itemBuilder: (context, i) {
                        ...
                        return _TransaksiCard(
                          ...
                          metodeTone: isLunasDiMuka(p.metodeBayar) ? BadgeTone.green : BadgeTone.amber,
    ...
    return Scaffold(
      appBar: widget.embedded
          ? null
          : AppBar(
              title: const Text('Penjualan Barang'),
              bottom: _searchBar(),
            ),
