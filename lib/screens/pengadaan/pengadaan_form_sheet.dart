...
const _kNewSupplier = -1;
const double _kPpnRate = 0.11;

class PengadaanFormSheet extends StatefulWidget {
  final Pengadaan? existing;
  const PengadaanFormSheet({super.key, this.existing});

  @override
  State<PengadaanFormSheet> createState() => _PengadaanFormSheetState();
}

class _PengadaanFormSheetState extends State<PengadaanFormSheet> {
  final _barangService = BarangService();
  final _partnerService = PartnerService();
  final _pengadaanService = PengadaanService();
  ...
  Future<void> _loadRef() async {
    try {
      final results = await Future.wait([_barangService.list(), _partnerService.listSupplier()]);
      setState(() {
  ...
  Future<void> _addSupplierDialog() async {
    ...
    final result = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            TextField(controller: namaCtrl, autofocus: true, decoration: const InputDecoration(labelText: 'Nama Supplier')),
    ...
    try {
      final s = await _partnerService.createSupplier({
      ...
      setState(() {
        _suppliers = [..._suppliers, s];
        _supplierId = s.id;
      });
    } catch (e) {
      if (mounted) setState(() => _error = e is ApiException ? e.message : 'Gagal menambah supplier');
    }
  }
  ...
  void _onBarangSelected(int idx, Barang b) {
    setState(() {
      ...
      _rows[idx].hargaSatuan = b.hargaBeli;
  ...
  Future<void> _submit() async {
    ...
    try {
      if (_editing) {
        await _pengadaanService.update(widget.existing!.id, data);
      } else {
        await _pengadaanService.create(data);
      }
          ...
          child: Column(
            children: [
              Padding(
                ...
                child: Row(children: [
                  Expanded(
                      child: Text(_editing ? 'Edit Transaksi Pengadaan' : 'Transaksi Pengadaan Baru',
                          ...
                          children: [
                            DropdownButtonFormField<int>(
                              ...
                              items: [
                                ...
                                ..._suppliers.map((s) => DropdownMenuItem(value: s.id, child: Text(s.nama, overflow: TextOverflow.ellipsis))),
                                const DropdownMenuItem(value: _kNewSupplier, child: Text('+ Tambah Supplier Baru...')),
                              ],
                              onChanged: (v) => v == _kNewSupplier ? _addSupplierDialog() : setState(() => _supplierId = v),
                            ),
                            ...
                            const SizedBox(height: 10),
                            ..._rows.asMap().entries.map((entry) => _ItemRow(
                                  item: entry.value,
                            ...
                            DropdownButtonFormField<MetodeBayar>(
                              ...
                              items: [MetodeBayar.tunai, MetodeBayar.kredit, MetodeBayar.tempo]
                                  .map((m) => DropdownMenuItem(value: m, child: Text(metodeBayarLabel(m))))
                                  .toList(),
                            ...
                            if (_metodeBayar != MetodeBayar.tunai)
                              const Padding(
                                ...
                                child: Text(
                                  'Transaksi ini akan tercatat sebagai hutang ke supplier — bisa dilunasi lewat menu Akuntansi > Hutang & Piutang.',
